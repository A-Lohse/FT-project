...
def df_func(path):
    ...
    #get meta speech data
    for i in (root[0]):
        if i.tag == "ParliamentarySession":
            ParliamentarySession = i.text
        elif i.tag == "ParliamentaryGroup":
            ParliamentaryGroup = i.text
        elif i.tag == "MeetingNumber": 
            MeetingNumber = i.text
        elif i.tag == "DateOfSitting":
            DateOfSitting = i.text
            print(i.text)
        elif i.tag == "Location":
            Location = i.text
        else:
    ...
    df['start'] = start
    df['end'] = end
    df['ParliamentarySession'] = ParliamentarySession
    df['ParliamentaryGroup'] = ParliamentaryGroup
    df['MeetingNumber'] = MeetingNumber
    df['DateOfSitting'] = DateOfSitting
